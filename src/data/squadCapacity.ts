...
// Squad definitions — composição vigente a partir de Maio/2026
export const SQUAD_CAPACITY: SquadCapacityConfig[] = [
  ...
  },
  {
    ...
    members: [
      ...
      { name: "Tássio", role: "Arquiteto", cross: false, allocation: 1 },
...
// Composição legada — válida até Abril/2026 (inclusive)
export const SQUAD_CAPACITY_LEGACY: SquadCapacityConfig[] = [
  {
    name: "Golden Gate",
    product: "Contencioso Judicial",
    members: [
...
/**
 * Retorna a composição de squads vigente em um determinado mês (YYYY-MM).
 * A nova composição (sem Alexandre, Tássio full-time na JRE) entra em vigor em Maio/2026.
 */
export function getSquadCapacityForMonth(yearMonth: string): SquadCapacityConfig[] {
  if (!yearMonth) return SQUAD_CAPACITY;
  return yearMonth >= "2026-05" ? SQUAD_CAPACITY : SQUAD_CAPACITY_LEGACY;
}
